...
def clean_invalid_elem(xlist: List[str]) -> List[str]:
    return [i.strip() for i in xlist if i.strip() != '']
...
class Comtrade:
    ...
    def _load_inf(self, inf_file, **kwargs):
        if os.path.exists(inf_file):
            ...
            '''
            if kwargs.get("encoding"):
                kwargs["encoding"] = kwargs.get("encoding")
            else:
                with open(inf_file, 'rb') as file:
                    raw_data = file.read()
                    encoding = chardet.detect(raw_data)['encoding']
    ...
    def _read_mixed_text_bin_data_as_text(cff_file) -> str:
        ...
        while True:
            ...
            if current_value != chunk_delimiter:
                ...
                encoding = chardet.detect(cff_file.read())['encoding']
                yield b"".join(current_chunk).decode(encoding, errors="ignore").strip()
                current_chunk = []
...
class _BinaryDatReader(_DatReader):
    ...
    def get_reader_format(self, analog_channels, status_bytes):
        ...
        # print(analog_channels, dcount)
        # print(struct.calcsize(self.STRUCT_FORMAT.format(acount=analog_channels,
        #                                                 dcount=dcount)))
    ...
    def parse(self, contents):
        ...
        # print(type(contents.read()), len(contents.read()), struct.calcsize(contents.read()))
        # Row reading function.
        if isinstance(contents, io.TextIOBase) or \
                isinstance(contents, io.BufferedIOBase):
            # Read all_dataset buffer contents
